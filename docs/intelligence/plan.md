...
| R11 | **Peers are compared by exact weapon tag only; the game-mode dimension is removed.** There is one cohort definition: every other player's eligible kills with the same exact `kills.cause` tag, across all modes and maps, within the baseline lookback window and the reader's server scope, the subject excluded. `baseline.cohort` is gone from the config schema and `DEFAULT_CONFIG`; nothing groups or filters by mode, experiences or map, and a kill with no linked match (or one whose link is not credible) is compared like any other. This supersedes the mode/map cohort control in §4, the mode/map grouping and "unknown mode is its own unavailable comparison" in §5 *Peer cohorts*, the mode wording in acceptance tests 1 and 5, and `baseline.cohort` in the appendix `config.ts`. Unchanged: the sample minimums and the insufficient-sample state; recorded-match K/D, match history and session history; and burst timing, which still places kills on a validated match clock and leaves out of the burst (never the comparisons) any kill it cannot place. | Splitting by mode divides an already small sample, and kills without a linked match were being excluded from comparison. |
